<?php
namespace LFPackage\TestVs;

use Illuminate\Support\ServiceProvider;
use LFPackage\TestVs\Providers\RouteServiceProvider;

class TestVsServiceProvider extends ServiceProvider
{
    /**
     * 执行顺序: 1th
    ...
    public function register()
    {
        ...
        $this->mergeConfigFrom(
            __DIR__.'/../config/testvs.php', 'testvs'
        );
    ...
    {
        // 初始化 view 相关
        $this->loadViewsFrom(__DIR__.'/../resources/views', 'testvs');

        // 如果用户选择 publish, 那么系统会使用 publish 之后的 view, 而不是package 内的 view
        $this->publishes([
            __DIR__.'/../resources/views' => resource_path('views/vendor/testvs'),
        ], 'view');

        // 注意 laravel 只会将 public 下的文件作为资源文件加载, 所以对于资源文件来说, 必须
        // 在开发时, 需要在 public 文件夹下创建 vendor/testvs 文件夹
        // 在发布时, 将 public/vendor/testvs 下的内容丢到 package 的 resources/assets 文件夹中
        $this->publishes([
            __DIR__.'/../resources/assets' => public_path('vendor/testvs'),
        ], 'view');

        // 如果用户选择 publish,那么系统会读取 publish 之后的 config, 而不是 package 内的 config
        $this->publishes([
            __DIR__.'/../config/testvs.php' => config_path('testvs.php'),
        ], 'config');

        //如果模块有路由则将路由发布到项目中
        $routePrefix = config("testvs.route.prefix");
        $this->publishes([
        ...
        // migrate 指定路径: php artisan migrate --path="database/migrations/testvs" --refresh
        // 同样的如果需要 rollback, 也可以在 rollback 的时候加上 path 参数
        $this->publishes([
            __DIR__.'/../database/migrations/' => database_path('migrations/testvs')
        ], 'db');

        $this->publishes([
            __DIR__.'/../database/seeds/' => database_path('seeds/testvs')
        ], 'db');

        $this->publishes([
            __DIR__.'/../database/factories/' => database_path('factories/testvs')
        ], 'db');
